...
resource "azurerm_public_ip" "netstat" {
    name                         = "${var.prefix}netstat-ip-public"
    location                     = "${var.region}"
...
resource "azurerm_network_security_group" "netstat" {
    name                = "${var.prefix}netstat-security-group"
    location            = "${var.region}"
    resource_group_name = "${var.resource_group_name}"
...
resource "azurerm_network_interface" "netstat" {
   name                      = "${var.prefix}netstat-network-card"
   location                  = "${var.region}"
   resource_group_name       = "${var.resource_group_name}"
   network_security_group_id = "${azurerm_network_security_group.netstat.id}"

   ip_configuration {
       name                          = "${var.prefix}netstat-ip-private"
...
resource "azurerm_virtual_machine" "netstat" {
    ...
    storage_os_disk {
        name              = "${var.prefix}netstat-disk-os"
        caching           = "ReadWrite"
